...
describe('Test de RUTAS', () => {
    describe('GET/rickandmort/character/:id', () => {
        ...
        it('Si hay un error responde con status: 500', async () => {
            await agent.get('/rickandmorty/character/1777').expect(500);
        });
    })
    describe('GET /rickandmorty/login', () => {
        it('La informacion de login es correcta', async () => {
            const response = (await agent.get('/rickandmorty/login?email=leomicale.lm@gmail.com&password=leo123')).body;
            expect(response.access).toEqual(true);
        });
        it('La informacion de login es incorrecta', async () => {
            const response = (await agent.get('/rickandmorty/login?email=leomicale.m@gmail.com&password=leo123')).body;
            expect(response.access).toEqual(false);
        });
    });
    describe('POST /rickandmorty/fav', () => {
        let character1 = {id: '1', name: 'Leo'}
        let character2 = {id: '2', name: 'Mica'}
    ...
    describe('DELETE /rickandmorty/fav/:id', () => {
        let character1 = {id: '1', name: 'Leo'}
        let character2 = {id: '2', name: 'Mica'}
        it('Devuelve el arreglo conrrespondiente si no se elimina ningun personaje', async () => {
            const response = (await agent.delete('/rickandmorty/fav/1777')).body;
            expect(response).toContainEqual(character1);
            expect(response).toContainEqual(character2);
        })
